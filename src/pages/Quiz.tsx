        ...
        {!isLastQuestion ? (
          <>
            <div className='row'>
              <h5>Quiz App</h5>
                <div className='col-3'>
                  <button className='btn btn-primary' onClick={() => setIsRunning(prev => !prev)}>
                      {isRunning ? "⏸ Pausar" : "▶ Iniciar"}
                  </button>
                </div>
                <div className='col-3'>
                  <p className='fw-bold'>{time}s</p>
                </div>
            ...
            {currentQuestion.options.map((option, index) => {
              return (
                  <div className={`border p-3 mb-3 ${selectedIndex === index ? "bg-blue text-white selected" : ""}`}
                    key={index} 
            ...
            <div className='btn-send'>
              <button className={`btn btn-primary ${selectedIndex !== -1 ? "" : "disabled"}`}
